    ...
    return (
        ...
        <PaginationStyles>
            ...
            <div className="right_box_info">
                ...
                <button
                    className={`right-row ${pageNumber >= (Math.ceil(totalHits / perPage)) ? "desactive-btn" : "active-btn"}`}
                    onClick={() => pageNumber < Math.ceil(totalHits / perPage) && changeThePage(pageNumber + 1)}
                >
...
const mapDispatchToProps = dispatch => ({
    ...
    startLoadPageVideos: (kind, searchkey, page, perpage) => dispatch(loadVideos(kind, searchkey, page, perpage)),
});
